...
const Verify = () => {
    ...
    const verify = () => {
        ...
        confirmationResult.confirm(formik.values.code).then(() => {
            dispatch(registerUserEn({ name: formValues.name, email: formValues.email, password: formValues.password, phoneNumber: formValues.phoneNumber, role: formValues.role }));
        }).catch(() => {
            setLoading(false);
